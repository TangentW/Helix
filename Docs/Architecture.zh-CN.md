...
- 可表示的 managed Collection 共用一个经过验证的物化边界，而不是导入 Swift 私有的泛型 Collection ABI：Array 直接复用 storage，Set 与 Dictionary 按确定的 VM 迭代顺序复制成 `Array<Element>`，其中 Dictionary 暴露原生 `(Key, Value)` element Tuple。`enumerated`、`Array(sequence)`、异构 `zip`、极值、跨容器 Sequence 关系以及自然或 comparator 驱动的非可变排序都复用这一边界与现有强类型 Array 算法。reversed、repeated、slice 与 joined 在涉及更强的 index 或嵌套 Sequence 语义时仍保持 Array-backed 约束。归一化只覆盖元素序列语义；ArraySlice 的非零起始 index identity 不会被静默当成 Array index，尚未建模的 slice-index API 仍会 fail closed。
- 有限整数 `Range`/`ClosedRange` 与受支持数值 `StrideTo`/`StrideThrough` 构成第二类仅存在于 Compiler 的具体 Sequence specialization。它们保留强类型 bounds 与 stride register，而不产生 Swift Runtime 对象。正向高阶操作直接流式遍历；只有 `Array(sequence)`、`enumerated`、`reversed`、`zip` 这类结果需要存储或随机访问的 API，才会通过现有强类型 Array builder 物化元素。
