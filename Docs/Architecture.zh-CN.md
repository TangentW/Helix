...
- 冻结 Shell entry 除参数类型、结果与 effect 外，也包含精确的参数所有权 convention。未改动 callable 被用作 Swift closure 时只记录其 `EntryIndex`，不会复制 archive 中的旧函数体，也不会保存进程指针；closure context 仅保存已经验证的后缀捕获，普通、throwing 与 NativeImport callback 调用都通过同一条固定 generation 路由重新进入。
