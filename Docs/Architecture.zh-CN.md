...
- 有限整数 `Range`/`ClosedRange` 与受支持数值 `StrideTo`/`StrideThrough` 构成第二类仅存在于 Compiler 的具体 Sequence specialization。它们保留强类型 bounds 与 stride register，而不产生 Swift Runtime 对象。正向高阶操作、等值 membership、自然极值与 Sequence 关系直接流式遍历。整数 `Range`/`ClosedRange` 的 `count`、`isEmpty`、`first`、`last` 直接由 bounds 计算；完整 64 位基数使用无符号 order key，不遍历区间，并在精确结果无法放入 `Int` 时 trap。具有可表示 Comparable bounds 的 `Range` 也可执行 `isEmpty`，但不会因此获得迭代语义。自然/comparator 排序、Set 构造/代数以及其他结果需要完整存储的 API 才会通过同一强类型 Array builder 物化元素。
