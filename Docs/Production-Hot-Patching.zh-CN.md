...
3. 生成 Derived Sources，其中包含永久声明 Bridge、需要词法执行时按 hash 精确定位的源码 body wrapper，以及允许的原生 invoker；手写源码保持不变，Shell 编译仅对 observer 与 async 入口使用 derived copy 变换。
...
这里对 async accessor 的拒绝只针对 Shell accessor root。完全具体、签名可桥接的 async getter 可以冻结为 async NativeImport，但不会同时成为 Shell accessor root。
