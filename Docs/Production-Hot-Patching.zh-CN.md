...
NativeImport 还可通过同一套生成式 adapter 接受精确的直接或 Optional callback 参数，Swift closure 与 Objective-C block 共用该路径。v1 profile 只接受同步、nonthrowing、返回 `Void` 的 callback，保留 nonescaping/escaping 与 global-actor 合同，并只允许递归可桥接的 callback 参数；直接或 Optional `Error` callback 参数以有界 opaque proxy 跨边界，只携带动态类型文本名，不携带原生 payload、类型 metadata 或语义 identity，Shell entry 以及普通 NativeImport 参数/结果仍会拒绝 `Error`。escaping handle 会保留 image 与 generation lease；在通用 Swift `Sendable` 语义实现前，Runtime 会串行化 callback 执行。经过检查的源码默认参数投影使 UIKit animation/transition/property-animator、Dispatch queue/group、OperationQueue、Timer、URLSession 与 NotificationCenter callback 无需逐 API 的 VM 实现。Swift `Any` boxing、Objective-C protocol 擦除与 Foundation value-overlay bridge 同样冻结为精确且通用的 NativeImport adapter，而不是 API 特有的 Runtime 行为。ABI、schema、capability 与产品版本全部继续保持 1/1.0。
