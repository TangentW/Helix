...
文本能力不再局限于 Character 字面量判断。String/Character/Substring 使用显式逻辑合同：Character producer 与 Shell Bridge 验证恰好一个扩展字素簇，Substring 归一为 Character Array；String 的 `count`/`isEmpty` 不分配，需要 element 的遍历、变换、关系、基于数量的 subsequence、split、`Array(sequence)` 与 reversal 只物化一次并复用现有有限 Sequence plan。String/Substring 转换、Character 序列构造、嵌套 Character flatten、String 序列 joining、append/repetition、String/Character 比较与文本插值只使用两条 Verifier 可见的表示原语，不依赖 Swift 泛型 NativeImport 或逐源码 API opcode。首尾/计数删除、`popLast`、清空与容量提示则在 String、Substring、Array 和已归一的 Array-backed view 之间复用一个可表示 `RangeReplaceableCollection` 变更计划。私有 String index、UTF view、index-sensitive 修改与 Foundation 文本语义仍会 fail closed。
