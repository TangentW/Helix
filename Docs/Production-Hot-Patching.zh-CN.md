...
可表示的 managed Array、Set 与 Dictionary 还会通过一个通用物化边界复用极值、跨容器 Sequence 关系、非可变排序、`enumerated`、`Array(sequence)` 与异构 `zip`；Dictionary element 保持 `(Key, Value)` Tuple，Set/Dictionary 顺序使用确定的 VM 迭代顺序。可变排序仍只支持 Array。
