...
String 在一次受验证的 Character Array 物化后也复用这套 closure CFG；保留容器的 `filter` 会通过通用 builder 收尾为 String。String 与 Array-backed 来源支持 Collection `prefix/drop(while:)` 和反向 `last(where:)`，而 Sequence `prefix(while:)` 的直接 Array 结果形式还接受受支持有限 progression。String 的首尾/计数删除与 `popLast` 按扩展字素簇执行，并同 Substring/Array-backed storage 的相同操作共用结构性变更计划；可变排序仍只属于 Array。
