...
已表示标准值的泛型约束不会伪造 witness table：Compiler 使用经过当前 toolchain 校验的闭合 `Sequence`/`Collection` 层级，只贡献 Helix 已执行语义能够证明的 conformance 与 associated identity；自定义值不会因 storage 形状相似而继承这些协议。
