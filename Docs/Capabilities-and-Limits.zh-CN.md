...
- 对可表示 payload 支持直接 Optional 强制解包；nil 强制解包保留专用且经过验证的 trap reason。没有 payload 的运行时 `nil` 会从已验证的静态上下文获得 `T`，并统一覆盖嵌套 Tuple、collection builder、mutation/sort/split state、Dictionary key/value 与 VM equality。
