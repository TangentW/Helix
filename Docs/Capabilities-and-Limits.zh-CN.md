...
- Dictionary key 或 Set element 的用户自定义 `Hashable` 语义、VM-owned `Any` 内的动态 Set payload，以及 Character/Substring 的动态类型身份（包括嵌套 aggregate 中的出现）。Character/Substring 有意与 String/Array<String> 共用紧凑物理 storage；在 `Any` 携带递归逻辑类型 discriminator 前，这些装箱与转换会 fail closed。强类型 Set 与文本 Shell bridge 不受影响。
