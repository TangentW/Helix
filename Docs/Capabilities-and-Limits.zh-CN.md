...
- 有限具体整数 `Range`/`ClosedRange` 与受支持数值 stride 会复用 managed Collection 的正向高阶遍历，覆盖 `map`、`flatMap`、`filter`、`compactMap`、`reduce`、`reduce(into:_:)`、`forEach`、`first(where:)`、`contains(where:)`、`allSatisfy` 以及 comparator 驱动的 `min(by:)`/`max(by:)`；产生新容器的变换返回 Array，并保留相同的短路、抛错和可变捕获语义。它们还支持等值 `contains(_:)`、自然 `min()`/`max()`、与 managed Collection/另一个 progression 混合的 Sequence 关系、自然/comparator `sorted`、`Set(sequence)`，以及 sequence operand 已完全具体且有限的泛型 Set 代数。只消费 element 的操作保持短路或单 candidate 流式执行；排序与 Set 结果才通过统一强类型 builder 物化。`Array(sequence)`、`enumerated()`、`reversed()` 与异构 `zip` 同样只在结果需要存储或随机访问时物化 progression。反向 predicate 搜索、返回 index 的搜索、index-sensitive Collection boundary/subsequence 与无界 partial range 会在方向、index identity、复杂度或终止条件能够精确表示前继续 fail closed。
