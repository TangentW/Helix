...
- 有限具体整数 `Range`/`ClosedRange` 与受支持数值 stride 会复用 managed Collection 的正向高阶遍历，覆盖 `map`、`flatMap`、`filter`、`compactMap`、`reduce`、`reduce(into:_:)`、`forEach`、`first(where:)`、`contains(where:)`、`allSatisfy` 以及 comparator 驱动的 `min(by:)`/`max(by:)`；产生新容器的变换返回 Array，并保留相同的短路、抛错和可变捕获语义。`Array(sequence)`、`enumerated()`、`reversed()` 与异构 `zip` 只在结果需要存储或随机访问时，通过同一强类型 builder 物化 progression。反向 predicate 搜索、返回 index 的搜索与无界 partial range 会在方向、index identity 或终止条件能够精确表示前继续 fail closed。
