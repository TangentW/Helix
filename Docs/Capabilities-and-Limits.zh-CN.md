...
- `String`、`Character` 与 `Substring` 使用显式逻辑文本合同，不导入标准库私有布局。表示层 `Character` 在每个 producer 或 Shell 边界都会验证为恰好一个扩展字素簇；`Substring` 归一为 Character 序列，不保留私有 slice storage 或 index identity。已支持字面量、相等/排序、拼接、`+=`、追加 String/Character、标量和文本插值、重复与常见 String 构造、Unicode `uppercased`/`lowercased`、count/empty、prefix/suffix/contains、String/Substring 转换、Character 序列构造、String 序列 joining，以及按扩展字素簇执行的 `removeFirst`/`removeLast`（含计数形式）、`popLast`、清空与容量提示。这些变更与 Array-backed 值共用一个可表示 `RangeReplaceableCollection` 计划，不增加文本专用字节码。String 直接执行 `count`/`isEmpty`；需要 element 的 `first`/`last`、变换、关系、`Array(sequence)`、`reversed`、基于数量的 subsequence 与 split 会经验证归一为 Character Array，再进入现有有限 Sequence cursor、adapter 与 split 状态。嵌套可表示 Character 序列也可 flatten 后重建为 String。可变大小文本操作会预扣确定性的 UTF-8 工作量与输出存储。
