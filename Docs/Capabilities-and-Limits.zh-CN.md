...
- `PartialRangeFrom`、`PartialRangeUpTo`、`PartialRangeThrough` 的 containment（包括 switch pattern 产生的 `RangeExpression.~=`）对可表示整数、浮点、String 与 Character bounds 共用一套强类型比较计划；动态无序浮点 bound 仍保留 Swift 构造 trap。具体 Array 以单侧 `Int` bound 下标时复用既有 suffix/prefix subsequence 操作；全范围 `[...]` marker 则在可表示 Array-backed 或 String/Substring 来源通过既有物化边界归一后被消除。这些范围值只存在于 Compiler：不会序列化 Swift 泛型集合 ABI，不会把标准库方法注册为 NativeImport，也不会新增逐源码 API opcode。自定义 `Comparable` witness、私有 `String.Index` 与派生 ArraySlice index 仍会 fail closed。
