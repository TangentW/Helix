...
- 受管 Debug 会测量所有“已经冻结 imported native type”所属 module 的公开可读 type property。捕获 toolchain 的 symbol graph 负责提名满足 minimum OS 的 API，同一 typed AST/canonical SIL 流程只冻结唯一且 Bridge-compatible 的 getter。这条通用路径覆盖 Swift 与 Objective-C 声明、Swift overlay rename 和 SDK actor 隔离；例如 `UIColor.black`、`UIScreen.main`、`UIView.areAnimationsEnabled`、`Bundle.main` 与 `ProcessInfo.processInfo`。生产 Shell 不会得到这组便利能力，它本身也不会引入新的 owner 或结果类型。
...
| 在受管 Debug body 中首次使用公开可读 type property | owner 与结果已能由冻结 imported/Bridge 类型面表达、SDK 声明满足 Shell minimum OS，且捕获 frontend 能测得唯一 getter ABI 时支持；常见 UIKit/Foundation 示例包括 `UIColor.black`、`UIScreen.main` 与 `Bundle.main` |
