...
> **项目状态：** 当前仓库是一套可执行技术基线，不代表已经支持任意 Swift，也不代表已经获得 App Store 热补丁下发资格。生产 App Store 通道被明确阻断；真机、业务 corpus、长期 soak 和控制面 Gate 仍未关闭。
...
| 开发期热重载 | 会话绑定的签名 dylib；符合条件时也可用开发期 HLBC | Debug App 中的 Swift Dynamic Replacement 或开发期 HLVM 路径 | 保存 Swift 函数体后刷新正在运行的 UIKit/SwiftUI 页面，无需重新安装 |
...
flowchart LR
    ...
    C --> D["开发：typed AST replacement → 签名 dylib"]
    P --> R["HelixAppRuntime"]
...
- 使用精确工具链把已声明 Swift 子集编译成 HLBC：常用标量与 String、Array/Dictionary 值语义、局部 struct/enum 与具体 `Result`、带 payload 的局部错误、受限局部 `inout`/`mutating`、同步非逃逸 closure、具体化 compiler specialization 与无 suspension async entry。
...
- 精确 Debug frontend job 捕获与复放、稳定保存快照、单调调度、Native/HLBC 路由、签名 Native image、认证传输、`dlopen`、dSYM 验证、UIKit 刷新、SwiftUI pulse boundary 与 Debug Overlay。
...
| Debug / Dev Shell | `HelixDevAppRuntime` | Release 能力，加上 Dev Protocol、Native Loading 与 UI Reload |
...
可执行参考见 [Demo/HelixDemo.xcodeproj](Demo/HelixDemo.xcodeproj) 与 [Demo/README.md](Demo/README.md)。Hot Patch App 会通过 Simulator mock-download inbox 走签名包客户端链；Live Reload App 会走真实构建捕获、保存检测、认证传输、Native 激活与 UI 刷新。
...
- `Sources/HelixDevProtocol`、`HelixDevTools`、`HelixDevRuntime` 与 `HelixLiveReloadAPI`：开发会话、Native generation、激活与 UI Reload 合同。
