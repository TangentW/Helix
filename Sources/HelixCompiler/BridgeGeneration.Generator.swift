...
public struct Generator: Sendable {
    ...
    public func generate(
        ...
        for source in sourceGroups {
            ...
            let externalTypeModules = Set(typeValues.compactMap {
                $0.1.nativeModuleName
            })
            guard externalTypeModules.count <= 1,
                  externalTypeModules.isEmpty
                    || (values.isEmpty && frozenValues.isEmpty
                        && typeValues.allSatisfy {
                            $0.1.nativeModuleName == externalTypeModules.first
                        })
            else {
                throw BridgeGeneration.Error.incompleteNativeTypeBindings
            }
    ...
    private func validateGeneratedNativeType(
        ...
        if let nativeModuleName = generated.nativeModuleName {
            originIsValid = binding.importedModules.contains(nativeModuleName)
                && !archive.sources.contains(where: {
                    $0.logicalPath == generated.sourceFileLogicalID
                })
        } else {
            originIsValid = archive.sources.contains(where: {
                $0.logicalPath == generated.sourceFileLogicalID
            })
        }
