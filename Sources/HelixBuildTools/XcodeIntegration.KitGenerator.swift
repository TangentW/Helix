...
public struct ProfileContract: Codable, Hashable, Sendable {
    ...
    public var bridgeConfiguration: String
    ...
    public init(
        ...
        bridgeModuleName = feature.bridgeModuleName
        let root = "Profiles/\(profile.id)"
        featureSourceList = "\(root)/FeatureSources.xcfilelist"
        bridgeSourceList = "\(root)/BridgeSources.xcfilelist"
        commonConfiguration = "\(root)/Profile.xcconfig"
        featureConfiguration = "\(root)/Feature.xcconfig"
        bridgeConfiguration = "\(root)/Bridge.xcconfig"
    }
}
...
public struct KitGenerator: Sendable {
    ...
    private func generate(
        ...
        try insert(
            Data(renderFeatureConfiguration(profile: profile, feature: feature).utf8),
            at: contract.featureConfiguration,
            into: &artifacts
        )
        try insert(
            Data(renderBridgeConfiguration(profile: profile, feature: feature).utf8),
            at: contract.bridgeConfiguration,
            into: &artifacts
        )
        try insert(
            Data(renderFeatureSourceList(feature: feature).utf8),
            at: contract.featureSourceList,
            into: &artifacts
        )
        try insert(
            Data(renderBridgeSourceList(feature: feature).utf8),
            at: contract.bridgeSourceList,
            into: &artifacts
        )
    ...
    private func renderFeatureConfiguration(
        ...
        let liveFlags = profile.workflow == .liveReload
            ? " -Xfrontend -enable-implicit-dynamic -Xfrontend -enable-dynamic-replacement-chaining"
            : ""
        let liveCompilerCapture = profile.workflow == .liveReload
            ? """
            HELIX_REAL_SWIFT_EXEC = $(TOOLCHAIN_DIR)/usr/bin/swiftc
            SWIFT_EXEC = $(HELIX_PROFILE_OUTPUT_DIR)/\(XcodeIntegration.CompilerCapture.proxyFileName)
        ...
        OTHER_SWIFT_FLAGS = $(inherited) -Xfrontend -enable-private-imports\(liveFlags)
        \(liveCompilerCapture)

        """.trimmingCharacters(in: .newlines) + "\n"
    }

    private func renderBridgeConfiguration(
        profile: XcodeIntegration.Profile,
        feature: XcodeIntegration.Feature
    ) -> String {
        """
        // Generated by Helix. Do not edit.
        #include "Profile.xcconfig"
        PRODUCT_MODULE_NAME = \(feature.bridgeModuleName)
        LD_DYLIB_INSTALL_NAME = @rpath/$(EXECUTABLE_PATH)
        OTHER_SWIFT_FLAGS = $(inherited) -Xfrontend -enable-private-imports -Xfrontend -enable-dynamic-replacement-chaining

        """
    }

    private func renderFeatureSourceList(feature: XcodeIntegration.Feature) -> String {
        feature.sourceFiles.map {
            "$(HELIX_SHELL_OUTPUT_DIR)/DerivedSources/"
                + SourceTransform.transformedFilePath(for: $0)
        }.joined(separator: "\n") + "\n"
    }

    private func renderBridgeSourceList(feature: XcodeIntegration.Feature) -> String {
        var paths = feature.sourceFiles.flatMap { source in
            [
                BridgeGeneration.Generator.entrySourcePath(for: source),
                BridgeGeneration.Generator.nativeImportSourcePath(for: source),
            ]
        }
        paths.append("Generated/\(feature.bridgeTypeName).swift")
        paths.append(ShellBuild.DevContractGenerator.sourcePath(moduleName: feature.moduleName))
        return Array(Set(paths)).sorted().map {
            "$(HELIX_SHELL_OUTPUT_DIR)/\($0)"
        }.joined(separator: "\n") + "\n"
    }
    ...
    private func phaseNames(for profile: XcodeIntegration.Profile) -> [String] {
        ...
        case .hotPatch:
            profile.patch == nil
                ? ["prepare", "finalize", "audit"]
                : ["prepare", "finalize", "audit", "patch"]
        case .liveReload:
            ["prepare", "finalize", "live-start", "live-stop"]
        }
    }
    ...
    private func renderGuide(
        ...
        let profiles = zip(orderedContracts, plan.profiles).map { contract, profile in
            ...
            - Link only `\(contract.runtimePackageProduct)` into the App and generated Bridge.
            - Use `\(contract.featureConfiguration)` as the Feature target base configuration.
            - Use `\(contract.bridgeConfiguration)` as the Bridge target base configuration.
            - Add every generated path in `\(contract.featureSourceList)` and
              `\(contract.bridgeSourceList)` to the corresponding Sources phase once.
